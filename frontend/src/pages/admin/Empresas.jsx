...
export default function AdminEmpresas() {
  ...
  const salvar = useMutation({
    mutationFn: async (dados) => {
      ...
      return editando
        ? api.put(`/admin/empresas/${editando}`, fd, cfg)
        : api.post('/admin/empresas', fd, cfg)
    },
    onSuccess: () => {
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {empresas.filter(e=>e.ativo).length} ativas · {empresas.filter(e=>!e.ativo).length} inativas
          </p>
        </div>
                ...
                <div className={`p-4 flex items-center gap-4 ${
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <span className={`inline-flex items-center gap-1 mt-1.5 text-xs px-2 py-0.5 rounded-full font-semibold ${
                      ativa ? 'bg-green-400/25 text-green-300' : 'bg-red-400/25 text-red-300'
                    }`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${ativa ? 'bg-green-400' : 'bg-red-400'}`}/>
                      {ativa ? 'Ativa — visível no site' : 'Inativa — oculta no site'}
                    </span>
                  </div>
                </div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); salvar.mutate(form) }} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" className="btn-primary flex-1" disabled={salvar.isPending}>
                  {salvar.isPending ? 'Salvando...' : editando ? 'Salvar alterações' : 'Criar associação'}
                </button>
              </div>
            </form>
          </div>
